/*
  WiFiConnectionAccessPointMode.h
...
// WiFi TCP Server connection implementation - Access Point Mode.
// The ESP32 creates its own WiFi Access Point using ssid/password
// and opens a TCP Server on the given port.
// An external client (e.g. PC, phone) connects to the AP and then to the Server.
class WiFiConnectionAccessPointMode final : public Connection
{

public:
    // Constructor - accepts the AP SSID, password and TCP Server port.
    // These values are typically defined as #define constants in the machine config file.
    explicit WiFiConnectionAccessPointMode(const char *ssid, const char *password, uint16_t serverPort);
    ~WiFiConnectionAccessPointMode() override = default;
    ...
    // Returns true if the WiFi Access Point was successfully started during Init().
...
private:
    // AP credentials and TCP Server port passed via constructor.
    String fSsid;
    String fPassword;
    ...
    // True after the WiFi Access Point has been successfully started during Init().
