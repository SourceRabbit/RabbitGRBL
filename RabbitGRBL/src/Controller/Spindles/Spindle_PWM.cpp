 ...
 */
void Spindle_PWM::InitializePWMOutput(uint8_t channelNumber, uint32_t frequency, uint8_t precision, uint32_t period, uint32_t offValue,
...
uint32_t Spindle_PWM::setRPM(uint32_t rpm)
{
...
void Spindle_PWM::setState(SpindleState state, uint32_t rpm)
{
    if (sys.abort)
    {
...
SpindleState Spindle_PWM::getState()
{
...
void Spindle_PWM::setPWMOutput(uint32_t duty)
{
    if (fOutputPin == UNDEFINED_PIN)
    {
        return;
...
void Spindle_PWM::setEnablePinValue(bool active)
{
    if (fEnablePin == UNDEFINED_PIN)
    {
        return;
...
void Spindle_PWM::setDirectionPinValue(bool Clockwise)
{
