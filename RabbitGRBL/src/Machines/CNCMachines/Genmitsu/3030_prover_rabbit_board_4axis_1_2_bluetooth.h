/**
 * Machine Configuration
 * Target Machine 		: Genmitsu 3030 ProVer
 * Motion Controller	: Rabbit Board 4-Axis (https://www.sourcerabbit.com/Shop/pr-i-106-t-rabbit-board-4-axis.htm)
 *
 * Description:
 * This configuration file defines the default motion parameters,
 * axis setup, spindle configuration, IO mapping and limits
 * for the Genmitsu 3030 ProVer CNC machine operating
 * with Rabbit GRBL in 4-axis mode.
 *
 * Notes:
 * - Designed for x16 microstepping drivers
 * - Soft limits enabled for XYZ axes
 * - A axis soft limit intentionally disabled (max travel = 0)
 * - Motors remain energized (no idle unlock)
 */

#pragma once

#define MACHINE_NAME "Genmitsu 3030 ProVer"

// Define Bluetooth
#define ENABLE_BLUETOOTH
#define BT_DEVICE_NAME "Genmitsu3030"

#ifdef N_AXIS
#undef N_AXIS
#endif
#define N_AXIS 4

#define DEFAULT_STEP_PULSE_MICROSECONDS 5  // $0
#define DEFAULT_STEPPER_IDLE_LOCK_TIME 255 // $1 KEEP MOTORS ALWAYS ON

#define DEFAULT_JUNCTION_DEVIATION 0.025 // $11 mm
#define DEFAULT_ARC_TOLERANCE 0.005      // $12 mm

#define DEFAULT_DIRECTION_INVERT_MASK 4 // $3 Invert X and Y
#define DEFAULT_INVERT_LIMIT_PINS 0     // $5 No
#define DEFAULT_INVERT_PROBE_PIN 0      // $6 No

#define DEFAULT_SOFT_LIMIT_ENABLE 1 // Soft Limits Disabled
#define DEFAULT_HARD_LIMIT_ENABLE 0 // Hard Limits Disabled
#define DEFAULT_HOMING_ENABLE 1
#define DEFAULT_HOMING_DIR_MASK 3       // $23 move positive dir Z, negative X,Y
#define DEFAULT_HOMING_FEED_RATE 100.0  // $24 mm/min
#define DEFAULT_HOMING_SEEK_RATE 1500.0 // $25 mm/min
#define DEFAULT_HOMING_PULLOFF 2.0      // $27

// TRAVEL
#define DEFAULT_X_MAX_TRAVEL 312.0 // $130 X axis Max Travel (mm)
#define DEFAULT_Y_MAX_TRAVEL 308.0 // $131 Y axis Max Travel (mm)
#define DEFAULT_Z_MAX_TRAVEL 80.0  // $132 Z axis Max Travel (mm)
#define DEFAULT_A_MAX_TRAVEL 0     // $133 A axis Max Travel (mm or degrees) (NOTE: We set this to zero in order to avoid Soft Limits for this Axis)

// SPEEDS AND ACCELLERATIONS (x16 Microstepping)
#define DEFAULT_X_STEPS_PER_MM 800.0 // $100 steps/mm
#define DEFAULT_X_MAX_RATE 3500.0    // $110 mm/min
#define DEFAULT_X_ACCELERATION 80.0  // $120 mm/sec^2

#define DEFAULT_Y_STEPS_PER_MM 800.0 // $101 steps/mm
#define DEFAULT_Y_MAX_RATE 3500.0    // $111 mm/min
#define DEFAULT_Y_ACCELERATION 80.0  // $121 mm/sec^2

#define DEFAULT_Z_STEPS_PER_MM 800.0 // $102 steps/mm
#define DEFAULT_Z_MAX_RATE 2500.0    // $112 mm/min
#define DEFAULT_Z_ACCELERATION 50.0  // $122 mm/sec^2

#define DEFAULT_A_STEPS_PER_MM 26.666 // $103 steps/mm or steps/degree
#define DEFAULT_A_MAX_RATE 7200.0     // $113 mm/min or degrees/min
#define DEFAULT_A_ACCELERATION 60.0   // $123 steps/sec^2 mm/sec^2

#define X_STEP_PIN GPIO_NUM_16
#define X_DIRECTION_PIN GPIO_NUM_33
#define Y_STEP_PIN GPIO_NUM_25
#define Y_DIRECTION_PIN GPIO_NUM_26
#define Z_STEP_PIN GPIO_NUM_27
#define Z_DIRECTION_PIN GPIO_NUM_14
#define A_STEP_PIN GPIO_NUM_12
#define A_DIRECTION_PIN GPIO_NUM_13
#define STEPPERS_DISABLE_PIN GPIO_NUM_15

#define SPINDLE_TYPE ESpindleType::RELAY
#define SPINDLE_OUTPUT_PIN GPIO_NUM_2
#define SPINDLE_DIR_PIN GPIO_NUM_21
#define DEFAULT_SPINDLE_RPM_MIN 0        // RPM
#define DEFAULT_SPINDLE_RPM_MAX 25000    // RPM
#define DEFAULT_SPINDLE_FREQ 5000        // Hz
#define DEFAULT_SPINDLE_DELAY_SPINUP 0   // Seconds
#define DEFAULT_SPINDLE_DELAY_SPINDOWN 0 // Seconds

#define X_LIMIT_PIN GPIO_NUM_36
#define Y_LIMIT_PIN GPIO_NUM_39
#define Z_LIMIT_PIN GPIO_NUM_34
#define A_LIMIT_PIN GPIO_NUM_35
#define PROBE_PIN GPIO_NUM_32

#define COOLANT_MIST_PIN GPIO_NUM_22
#define COOLANT_FLOOD_PIN GPIO_NUM_23

// Automatic Tool Change
#define USER_DIGITAL_PIN_0 GPIO_NUM_19 // Lock
#define USER_DIGITAL_PIN_1 GPIO_NUM_18 // Blow
#define USER_DIGITAL_PIN_2 GPIO_NUM_5  // Door