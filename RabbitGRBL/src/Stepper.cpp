...
static void stepper_pulse_func()
{
    ...
    if (st.exec_segment == NULL)
    {
        ...
        else
        {
            ...
            { // added to prevent ... jog after probing crash
    ...
    for (int axis = 0; axis < n_axis; axis++)
    {
        ...
        if (st.counter[axis] > st.exec_block->step_event_count)
        {
            ...
            if (!st.exec_block->backlash_motion)
            {
    ...
    { // Check if we need to fill the buffer.
        ...
        if (pl_block == NULL)
        {
            ...
            else
            {
                ...
                st_prep_block->backlash_motion = pl_block->motion.antiBacklashMotion;
...
void IRAM_ATTR Stepper_Timer_Init()
{
    ...
    config.divider = ESP32_TIMERS_CLOCK / fStepperTimer;
...
{
    // returns true if steppers are disabled
    bool disabled = false;
